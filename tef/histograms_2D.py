...
def grid_volume(ds, xislice, etaslice):
    '''
This function computes the grid cell volume for each boundary of a specified control volume. This is then used to
normalize the tracer fluxes so that the numbers actually have some context to them. 
    '''
    ...
    dVW = dVu.isel(xi_u = 0) #West face of control volume
    dVE = dVu.isel(xi_u = -1) #East face of control volume
    dVN = dVv.isel(eta_v = -1) #North face of control volume
    dVS = dVv.isel(eta_v = 0) #South face of control volume
    ...
    dVda = xr.merge([dVW, dVE, dVN, dVS], compat='override') #grid cell volume data array
    return dVda 
...
saltda = salt_cv(ds, grid, xislice, etaslice) #salt at the boundaries of the CV
depthda = depth_cv(ds, grid, xislice, etaslice) #vertical position in water column (z) at the boundaries of the CV
dVda = grid_volume(ds, xislice, etaslice) #volume a grid cell along the boundaries of the CV

Qda = volume_flux(ds, xislice, etaslice)
svarda,Qsvarda = Qcsvar_faces(ds, grid, saltda, Qda, xislice, etaslice)
Qsvarda['QsvarW'] = Qsvarda.QsvarW/dVda.dVW
Qsvarda['QsvarE'] = Qsvarda.QsvarE/dVda.dVE
Qsvarda['QsvarN'] = Qsvarda.QsvarN/dVda.dVN
Qsvarda['QsvarS'] = Qsvarda.QsvarS/dVda.dVS
Qda['QW'] = Qda.QW/dVda.dVW
Qda['QE'] = Qda.QE/dVda.dVE
Qda['QN'] = Qda.QN/dVda.dVN
Qda['QS'] = Qda.QS/dVda.dVS
...
depthbins = np.linspace(-65,0,51) #max depth or this control volume. Remember to change if you switch the box!!!
...
Qsvarh_da.to_netcdf(path = '../analysis/normalized/Qsvarh_nested_10min_2D_4.nc', engine = 'h5netcdf')
...
#     Qh_da.to_netcdf('../analysis/normalized/Qh_nested_10min_2D_zrho_%i.nc' % item, engine="h5netcdf")
